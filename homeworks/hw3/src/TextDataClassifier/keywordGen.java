...
public class keywordGen{
    ...
    public static String[] getKeywords(double[] vec, Dataset dataset){
        int len = vec.length;
        ...
        if (Classifier.debugMode)
            printArr(vec, sortedIndices);
        for (int i = 0; i < len; ++i){
            keywords[i] = dataset.tfidfObject.revIndices.get(sortedIndices[i]);
        }
        return keywords;
    }
    ...
    public static void generateKeywords(double[][] tfidfMatrix, Dataset dataset) throws Exception{
        ...
        for (FolderData folder : dataset.folders){
            ...
            for (DocumentData doc : folder.documents)
                kNN.addVec(vec, tfidfMatrix[doc.documentIndex]);
            String[] keywords = getKeywords(vec, dataset);
